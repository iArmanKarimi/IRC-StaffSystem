...
async function seedEmployees() {
	...
	if (!provinces || provinces.length === 0) {
		console.error(
			"No provinces found in the database. Please seed provinces first.",
		);
		process.exit(1);
	}
	...
	for (const province of provinces) {
		...
		const employees = Array.from({ length: 110 }, (_, i) => {
			const male = i % 2 === 0;
			const married = i % 3 === 0;
			...
			const nationalID = `NID${provinceId.toString().slice(-3)}${101000000 + i}`;
			...
			const gregorianBirth = jalaali.toGregorian(
				persianBirthYear,
				birthMonth,
				birthDay,
			);
			...
			const jobDay = (i % 28) + 1;
			const gregorianJob = jalaali.toGregorian(
				persianJobYear,
				jobMonth,
				jobDay,
			);
		...
		for (const empData of employees) {
			...
			await employee.save();
			console.log(
				`Seeded employee: ${empData.basicInfo.firstName} ${empData.basicInfo.lastName} for province ${province.name || province._id}`,
			);
		}
	}
