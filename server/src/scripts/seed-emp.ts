...
async function seedEmployees() {
	...
	// Fetch a real provinceId from the database
	const province = await Province.findOne();
	if (!province) {
		console.error('No province found in the database. Please seed provinces first.');
		process.exit(1);
	}
	const provinceId = province._id;

	// Generate 25 sample employees
	const employees = Array.from({ length: 25 }, (_, i) => {
		...
		const nationalID = `NID${100000000 + i}`;
		...
		const contactNumber = `09${(10000000 + i).toString().padStart(8, '0')}`;
	...
	for (const empData of employees) {
		...
		await employee.save();
		console.log(`Seeded employee: ${empData.basicInfo.firstName} ${empData.basicInfo.lastName}`);
	}
